      ...
      - rewrite !insert_insert. subst w'.
        ...
        + destruct (reg_eq_dec dst PC).
          * subst dst. rewrite lookup_insert// in H1; inv H1.
            replace (updatePcPerm (WCap asid p b e a)) with ((WCap asid p b e a):Word); [|destruct Hp; subst p; auto].
          ...
          * assert (H1' := H1). rewrite lookup_insert_ne in H1; auto.
            ...
            { subst p0. iDestruct ("Hreg" $! dst _ _ n H1 H1') as "Hinvdst".
              rewrite /interp (fixpoint_interp1_eq (WCap E _ _ _, WCap _ _ _ _ _)) /=.
              ...
              - iSplitR.
                ...
                + replace (<[PC:=WCap RX b0 e0 a0]> r2) with (<[PC:=WCap RX b0 e0 a0]> r1); auto.
                  ...
                  assert (<[PC:=WCap asid p b e a]> r1 !! i = <[PC:=WCap asid p b e a]> r2 !! i).
                  { rewrite Heq; auto. }
            ...
            { iDestruct ("Hreg" $! dst _ _ n H1 H1') as "Hinvdst".
              ...
              - replace (<[PC:=WCap asid p0 b0 e0 a0]> r2) with (<[PC:=WCap asid p0 b0 e0 a0]> r1); [destruct p0; auto; congruence|].
                unfold_leibniz. eapply map_eq. intros.
                destruct (reg_eq_dec PC i); [subst i; rewrite !lookup_insert; auto|rewrite !lookup_insert_ne; auto].
                assert (<[PC:=WCap asid p b e a]> r1 !! i = <[PC:=WCap asid p b e a]> r2 !! i).
                { rewrite Heq; auto. }
