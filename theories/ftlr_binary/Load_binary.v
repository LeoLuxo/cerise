...
Section fundamental.
  ...
  Lemma create_load_res:
    ∀ (r : leibnizO Reg) (p : Perm)
      ...
      (b0 e0 a0 : Addr),
      read_reg_inr (<[PC:=WCap asid p b e a]> r) src p0 b0 e0 a0

      → (∀ (r0 : RegName) v1 v2, (⌜r0 ≠ PC⌝  → ⌜r !! r0 = Some v1⌝ → ⌜r !! r0 = Some v2⌝ → (fixpoint interp1) (v1, v2)))
          -∗ ∃ P, allow_load_res src (<[PC:=WCap asid p b e a]> r) a a0 p0 b0 e0 P.
  Proof.
  ...
  Instance if_Persistent p b e a r src p0 b0 e0 a0 loadv: Persistent (if decide (reg_allows_load (<[PC:=WCap asid p b e a]> r) src p0 b0 e0 a0 ∧ a0 ≠ a) then interp loadv else emp)%I.
  Proof. intros. destruct (decide (reg_allows_load (<[PC:=WCap asid p b e a]> r) src p0 b0 e0 a0 ∧ a0 ≠ a));apply _. Qed.
  ...
  Proof.
    ...
    assert(∀ x : RegName, is_Some (<[PC:=WCap asid p b e a]> r.1 !! x)) as Hsome'.
    {
    ...
    assert (∃ p0 b0 e0 a0, read_reg_inr (<[PC:=WCap asid p b e a]> r.1) src p0 b0 e0 a0) as [p0 [b0 [e0 [a0 HVsrc] ] ] ].
    {
    ...
    { destruct (decide (reg_allows_load (<[PC:=WCap asid p b e a]> r1) src p0 b0 e0 a0 ∧ a0 ≠ a)); solve_ndisj. }
    ...
    { rewrite /allow_load_res /allow_load_mem.
      destruct (decide (reg_allows_load (<[PC:=WCap asid p b e a]> r1) src p0 b0 e0 a0 ∧ a0 ≠ a)).
