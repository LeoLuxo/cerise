 ...
 Context {Σ:gFunctors} {memg:memG Σ} {regg:regG Σ}
  ...
  Definition ftlr_instr (r : prodO (leibnizO Reg) (leibnizO Reg)) (p : Perm)
    ...
    -∗ ▷ □ (∀ w0 w'0 : Word, P (w0, w'0) -∗ interp (w0, w'0))
            ∗ (if decide (writeAllowed_in_r_a (<[PC:=WCap asid p b e a]> r.1) a) then ▷ □ (∀ w0 w'0 : Word, interp (w0, w'0) -∗ P (w0, w'0)) else emp)
    -∗ spec_registers_pointsto (<[PC:=WCap asid p b e a]> r.2)
    -∗ na_own logrel_nais ⊤
    -∗ ⤇ Seq (Instr Executable)
    -∗ a ↦ₐ w
    -∗ a ↣ₐ w'
    ...
    -∗ ([∗ map] k↦y ∈ delete PC (<[PC:=WCap asid p b e a]> r.1), k ↦ᵣ y)
