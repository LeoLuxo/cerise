...
Section fundamental.
  ...
  Lemma fundamental_cap r p b e a :
    ⊢ interp (WCap asid p b e a) →
      interp_expression r (WCap asid p b e a).
  Proof.
    iIntros "#Hinv /=".
    ...
    - (* Correct PC *)
      ...
      + (* Halt *)
        ...
        iExists (<[PC:=WCap asid p b e a]> r). iFrame.
        iAssert (∀ r0 : RegName, ⌜is_Some (<[PC:=WCap asid p b e a]> r !! r0)⌝)%I as "HA".
  ...
  Definition exec_cond b e p : iProp Σ :=
    (∀ a r, ⌜a ∈ₐ [[ b , e ]]⌝ → ▷ □ interp_expression r (WCap asid p b e a))%I.

  Lemma interp_exec_cond p b e a :
    p ≠ E -> interp (WCap asid p b e a) -∗ exec_cond b e p.
  Proof.
    iIntros (Hnp) "#Hw".
  ...
  Lemma exec_wp p b e a :
    ...
    exec_cond b e p -∗
    ∀ r, ▷ □ (interp_expr interp r) (WCap asid p b e a).
  Proof.
    iIntros (Hvpc) "#Hexec".
    rewrite /exec_cond.
    iIntros (r).
