...
Section cap_lang_rules.
  ...
  | Restrict_fail_allowed w:
      ...
      Restrict_failure regs dst src
  | Restrict_fail_invalid_perm_cap asid p b e a n:
      regs !! dst = Some (WCap asid p b e a) →
      p ≠ E →
      z_of_argument regs src = Some n →
      PermFlowsTo (decodePerm n) p = false →
      Restrict_failure regs dst src
  | Restrict_fail_PC_overflow_cap asid p b e a n:
      regs !! dst = Some (WCap asid p b e a) →
      p ≠ E →
      z_of_argument regs src = Some n →
      PermFlowsTo (decodePerm n) p = true →
      incrementPC (<[ dst := WCap asid (decodePerm n) b e a ]> regs) = None →
      Restrict_failure regs dst src
  ...
  | Restrict_spec_success_cap asid p b e a n:
      regs !! dst = Some (WCap asid p b e a) →
      p ≠ E ->
      z_of_argument regs src = Some n →
      PermFlowsTo (decodePerm n) p = true →
      incrementPC (<[ dst := WCap asid (decodePerm n) b e a ]> regs) = Some regs' →
  ...
  Lemma wp_Restrict Ep pc_asid pc_p pc_b pc_e pc_a w dst src regs :
  ...
  Proof.
    ...
    1,4: inversion Hwdst.
     ...
     + destruct (perm_eq_dec p E); [ subst p |].
       ...
       {
         assert (incrementPC (<[ dst := WCap asid (decodePerm wsrc) b e a ]> r) = None) as HH.
  ...
  Lemma wp_restrict_success_reg_PC Ep pc_asid pc_p pc_b pc_e pc_a pc_a' w rv z:
    ...
    PermFlowsTo (decodePerm z) pc_p = true →
     ...
     {{{ ▷ PC ↦ᵣ WCap pc_asid pc_p pc_b pc_e pc_a
       ...
       Instr Executable @ Ep
       {{{ RET NextIV;
           PC ↦ᵣ WCap pc_asid (decodePerm z) pc_b pc_e pc_a'
           ∗ (TEMP_virt_to_phys pc_a) ↦ₐ w
           ∗ rv ↦ᵣ WInt z }}}.
   Proof.
   ...
   Lemma wp_restrict_success_reg Ep pc_asid pc_p pc_b pc_e pc_a pc_a' w r1 rv asid p b e a z :
   ...
   Lemma wp_restrict_success_z_PC Ep pc_asid pc_p pc_b pc_e pc_a pc_a' w z :
   ...
   Lemma wp_restrict_success_z Ep pc_asid pc_p pc_b pc_e pc_a pc_a' w r1 asid p b e a z :
 ...
 Lemma wp_restrict_success_reg_sr Ep pc_asid pc_p pc_b pc_e pc_a pc_a' w r1 rv p b e a z :
     decodeInstrW w = Restrict r1 (inr rv) →
     isCorrectPC (WCap pc_asid pc_p pc_b pc_e pc_a) →
     (pc_a + 1)%va = Some pc_a' →
   ...
   Lemma wp_restrict_success_z_sr Ep pc_asid pc_p pc_b pc_e pc_a pc_a' w r1 p b e a z :
     decodeInstrW w = Restrict r1 (inl z) →
     isCorrectPC (WCap pc_asid pc_p pc_b pc_e pc_a) →
     (pc_a + 1)%va = Some pc_a' →
     SealPermFlowsTo (decodeSealPerms z) p = true →
